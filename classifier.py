...
class DINOClassifier(lightning.LightningModule):
    def __init__(self,
        ...
        self.classifier = Classifier(d_input=DINO_CLS_DIM, d_output=d_output)
        self.imsize = imsize
        self.crop_slices = DINOClassifier.__get_crop_slices(imsize, margin_tolerance, DINO_PATCH_SIZE)
    ...
    def forward(self, x):
        embedding = self.dinov2(x[..., self.crop_slices[-2], self.crop_slices[-1]])
